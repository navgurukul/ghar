...
async function getLearningPlatformData(reportName) {
  const token = await refreshToken();
  const apiUrl =
    "https://creator.zoho.in/api/v2.1/navgurukul/at-coder-cog-feb-15-2024/report/" +
    reportName;
  const headers = {
    Authorization: `Zoho-oauthtoken ${token}`,
    environment: "development",
  ...
  try {
    ...
    console.error("Error in getLearningPlatformData:", error);
    throw error;
  }
}

async function patchZohoDataForCopyOf(reportName, data) {
  const token = await refreshToken();
  const apiUrl =
    "https://creator.zoho.in/api/v2.1/navgurukul/at-coder-cog-feb-15-2024/report/" +
    reportName;
  ...
  try {
    ...
    // console.error("Error in patchZohoDataForCopyOf:", error);
    throw error;
  }
}
