...
async function getLearningPlatformData(reportName, checProdOrDev) {
  const token = await refreshToken();
  let apiUrl;
  let headers;

  if (checProdOrDev == true) {
    apiUrl =
      "https://creator.zoho.in/api/v2.1/navgurukul/at-coder-cog-feb-15-2024/report/" + reportName;
    headers = {
      Authorization: `Zoho-oauthtoken ${token}`,
      environment: "development",
      demo_user_name: "kajalzoho@navgurukul.org",
...
async function patchZohoDataForCopyOf(reportName, data, checProdOrDev) {
  const token = await refreshToken();
  let apiUrl;
  let headers;
