...
// ....................Zoho..Category..Update............................
  {
...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const data = await Services.categoryAndCoursesCopilot(email,checkProdOrDev); categoryAndCoursesCopilot
          // const data = await Services.categoryAndCourses(email,checkProdOrDev); categoryAndCoursesCopilot
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
...

    {
      ...
      handler: async (request, h) => {
        try {
          const { email,courseId,topicId,checkProdOrDev } = request.query;
          const data = await Services.topicAndActivities(email,courseId,checkProdOrDev,topicId); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
    ...
    // progress track of a user
    {
      ...
      handler: async (request, h) => {
        try {
          const { email,courseId,checkProdOrDev } = request.query;
          const data = await Services.progressTrack(email,courseId,checkProdOrDev); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
