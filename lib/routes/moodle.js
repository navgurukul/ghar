...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const data = await Services.categoryAndCoursesTest(emailArray,checkProdOrDev); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const data = await Services.topicAndActivitiesTest(emailArray,courseId,checkProdOrDev,topicId); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
