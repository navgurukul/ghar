...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const data = await Services.categoryAndCoursesTest(email,checkProdOrDev); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const { email ,checkProdOrDev} = request.payload;
          const data = await Services.coursesInMoodleWhereUserEnrolled(email,checkProdOrDev); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
...

    {
      ...
      handler: async (request, h) => {
        try {
          ...
          const data = await Services.topicAndActivitiesTest(email,courseId,checkProdOrDev,topicId); 
          return h.response(data);  // Using h.response to ensure proper response handling
        } catch (err) {
          console.error(err);
          return h.response({ error: 'Internal Server Error' }).code(500);
