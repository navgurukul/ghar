...
module.exports = [
  ...
  },
  {
    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        response = await Services.updateGrFrmGrStd(recordId, updateData, isDev);
        return h.response(response);
      } catch (err) {
        if (response.output) {
          return h
            .response({ error: response.output.payload.message })
  ...
  {
    method: "GET",
    path: "/gharZoho/All_Attendance_Configurations",
    options: {
      description: "Get the Attendance configuration value",
      notes: "config value",
      tags: ["api"],
      pre: [
        {
    ...
    handler: async (request, h) => {
      try {
        const { isDev } = request.query;
        // calling the getCampuses function from the service
        const response = await Services.getAttendanceConfigurationValue(isDev);
        return h.response(response);
      } catch (err) {
        return h.response({ error: err.message }).code(500);
      }
