...
module.exports = [
  {
    ...
    options: {
      description: "Get zoho Students data",
    ...
    options: {
      ...
      validate: {
        query: Joi.object({
          ...
          stdIdStart: Joi.number().required(),
          stdIdEnd: Joi.number().required(),
  ...
  {
    method: "GET",
    path: "/gharZoho/offer-response-placement-report",
    options: {
      description: "Get Offer_Response_and_Placement_Report by NavGurukul email list",
      notes: "Pass comma-separated emails in query string as emails",
      tags: ["api"],
      pre: [
        {
          assign: "auth",
    ...
    handler: async (request, h) => {
      try {
        const { emails, isDev } = request.query;
        const response = await Services.getOfferResponseAndPlacementByEmail(
          emails,
          isDev
        );
        return h.response(response);
