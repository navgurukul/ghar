...
class ReadTheoryService {
  ...
  // Fetch student data for given IDs with specified delay
  async fetchStudentData(studentIds) {
    const delay = 500;  // Set constant delay here
    ...
    const promises = studentIds.map((studentId, index) =>
      new Promise(resolve =>
        setTimeout(() => resolve(this.getStudentData(studentId, token, cookie)), index * delay)
      )
    );
  ...
  async getStudentData(studentId, token, cookie) {
    try {
      ...
      if (response && response.data && response.data.data && response.data.data.command) {
        ...
        return { [studentId.toString()]: studentData };
      } else {
        console.error(`Empty or unexpected response for studentId ${studentId}`, response.data);
        return { [studentId.toString()]: null }; // Return null or default structure for invalid IDs
      }
    } catch (error) {
      // console.error(`Error in getStudentData for studentId ${studentId}: ${error.message}`);
      return { [studentId.toString()]: null }; // Return null or default structure for errors
    }
  }
}
