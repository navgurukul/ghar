...
async function getScrapeData1(username) {
...
async function fetchAtcoderContestGet(contestId, username) {
  try {
    ...
    if (userInfo) {
      ...
      // console.log(count, "userInfo properties count");
...
async function fetchAtcoderContest1(contestId, usernames, delay) {
  try {
    const getCookies = await loginAndGetSession();
    const standingUrl = `https://atcoder.jp/contests/${contestId}/standings/json`;
    const response = await fetchWithRetry(standingUrl, {
...
/*const userData = require("../helpers/Atcoder");
async function getContestData() {
  try {
    ...
    for (const key of keys) {
      ...
      // console.log(`Contest: ${key}, Data:`, value);
      let storeData = await fetchAtcoderContest(value, key);
