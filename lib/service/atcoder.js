...
async function fetchAtcoderContest(contestId, username) {
  try {
    ...
    if (userInfo) {
      ...
      let UserData1 = {"UserScreenName":"shivamb22","Rank":8613,"TotalAccepted":0,"TotalScore":0,"Current_Rating":47,"Rating":"21","IsRated":true,"Contest Type":"Competition","contestId":"abc340","contestName":"KAJIMA CORPORATION CONTEST 2024（AtCoder Beginner Contest 340）","Date":"2024-02-10 22:40:00+0900","max_score":2625}
      return UserData1;
    } else {
      const UserData1 = {
        [username]: null,
  ...
  try {
    ...
    contestTable.children().each((index, element) => {
      ...
      if (dateCell && nameCell) {
        ...
        const contestName = nameCell.contents().filter((_, node) => node.nodeType === 3).text().trim(); // Extract contest name without markup
