...
async function getPreviousContestsByType(contestType, count) {
    try {
        const contestsData = await axios.get('https://kenkoooo.com/atcoder/resources/contests.json');
        
        const latestContestsList = contestsData.data
            .filter(contest => contest.id.startsWith(contestType))
            .sort((a, b) => b.start_epoch_second - a.start_epoch_second) // Sort in descending order
            .slice(0, count)
            .map(contest => contest.id);

        return { previousContests: latestContestsList };
    } catch (error) {
        throw new Error('Failed to fetch latest contests');
    }
}
module.exports = {
