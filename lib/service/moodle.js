  ...
  async progressTrack(studentNgEmails, courseId, progressTrack) {
    try {
      ...
      const progressTracks = await Promise.all(studentNgEmails.map(async (studentNgEmail) => {
        ...
        userEnrolledCourseTime[0].minDuration = userEnrolledCourseTime[0].courseMaxduration[0].value;
        userEnrolledCourseTime[0].maxDuration = userEnrolledCourseTime[0].courseMaxduration[1].value;
