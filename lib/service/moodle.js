    ...
    try {
      ...
      for (const email in userData) {
          if (userData.hasOwnProperty(email)) {
              const topicId = userData[email].finalResponse.response[0].id;
              const user = userData[email].finalResponse.response[0];
              const topicName = user.name;
              const totalActivity = user.totalActivity;
              const completedActivity = user.completedActivity;
              const complitionFlag = user.complitionFlag;
              const latestActivity = user.latestTime;
              const avgPercentage = user.avgPercentage;
