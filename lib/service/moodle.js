              ...
              try {
                  ...
                  const enrichedCourses = enrolledCourses.data.map(course => {
                      ...
                      return {
                        ...
                        minDuration: duration.customfields[0].value,
                        maxDuration: duration.customfields[1].value,
                    };
                  });
      ...
      for (let i of topicResponse) {
        for (let j of i.modules) {
          for (let k of completionStatus.completionstatus.completions) {
            ...
            const status = k.hasOwnProperty("complete") ? k.complete : false;
      ...
      const userResults = await Promise.all(
        emails.map(async (email) => {
          try {
            ...
            // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            // ...............calculate only summative assessment avg......................
            let SummativeAssAvg = 0;
            let activityCompletedInSummative = 0;
            let summativeAss = response.filter((topic) => {
              let lowerCaseTopicName = topic.name.toLowerCase();
              return lowerCaseTopicName.includes("summative assessment") || lowerCaseTopicName.includes("summative assessments");
            });
            if (summativeAss.length > 0) {
              for(let i of summativeAss[0].modules){
                if(i.grade.status == "true" && i.modname.toLowerCase() != "forum"){
                  activityCompletedInSummative += 1;
                }
                if(i.grade.percentageformatted !== ""){
                  SummativeAssAvg += parseInt(i.grade.percentageformatted);
                }
                if(i.grade.percentageformatted == "" && i.grade.status == "true"){
                  SummativeAssAvg += 100;
                }
              }
              console.log(SummativeAssAvg,activityCompletedInSummative)
              if(activityCompletedInSummative > 0){
                percentageAvg = SummativeAssAvg/activityCompletedInSummative;
              }
            }
