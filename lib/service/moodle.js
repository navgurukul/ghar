...
class Moodle2 {
  ...
  async fetchUserId(email, ip_address, wstoken) {
    ...
    const url = `http://${ip_address}/webservice/rest/server.php?wstoken=${wstoken}&wsfunction=core_user_get_users&moodlewsrestformat=json&criteria[0][key]=username&criteria[0][value]=${email}`;
  ...
  async progressTrack(userName, courseId, progressTrack) {
    try {
      ...
      const progressTracks = await Promise.all(userName.map(async (studentUserName) => {
        let userEnrolledCourseTime = userEnrolledCourseTimeResponse.data.filter(user => user.username.toLowerCase() == studentUserName.toLowerCase());
      ...
      const userResults = await Promise.all(emails.map(async (email) => {
        try {
          ...
          for (let topic of topicsResponse) {
            for (let module of topic.modules) {
              ...
              // if (activityStatus && activityStatus.completionstatus) {
              //   console.log(typeof activityStatus.completionstatus,'LLLLLLLL')
              //   module.completionStatus = activityStatus.completionstatus.filter((status)=>{
              //     console.log(status,'KKKKKKKKKKKKKKKK')
              //     return status.cmid === module.id
              //   }) ?.completionstatus; // Access completion status using optional chaining
              //   // module.completionStatus = activityStatus.completionstatus.filter(status => status.cmid === module.id)?.completionstatus; // Access completion status using optional chaining
              // } else {
          ...
          let response = await this.processTopicResponse(topicsResponse, activityStatus);

          let activityCompletedOfEachTopic  = response.reduce((count, item) => {
            return count + item.modules.reduce((subCount, module) => {
              return subCount + (module.grade.status=='true'&& module.modname.toLowerCase()!="forum" ? 1 : 0);
            }, 0);
          }, 0);
        ...
        for(let topic of response){
          ...
          for(let activity of topic.modules){
            ...
            //  finding min date among the activities...................
            const dates = minDate.map(dateStr => {
              if (dateStr) {
          ...
          const minDateArray = new Date(Math.min.apply(null, dates));
          
          // Format the minimum date
          const options = { day: '2-digit', month: 'short', year: 'numeric' };
          const minDateResult = minDateArray.toLocaleDateString('en-US', options);
         // Remove the comma from the date string
          const formattedDate = minDateResult.replace(',', '');
          // Split the date string into an array
          const dateParts = formattedDate!=undefined?formattedDate.split(" "):null
          // Reorder the date parts array to match the desired format
          ...
          topic.complitionFlag =checkFlag.length == topic.modules.length ? true : false;
          topic.latestTime = latestTime.length >20 ? null : latestTime;
          topic.minDate = reorderedDate.length >20 ? null : reorderedDate;
        }
        }     
