  ...
  async progressTrack(studentNgEmail, courseId) {
    try {
      ...
      userEnrolledCourseTime = userEnrolledCourseTime.data.filter((user) => {
        console.log(user.email, "user", studentNgEmail);
        const startTimeIST = new Date(user.firstaccess * 1000).toLocaleString(
          "en-IN",
          { timeZone: "Asia/Kolkata" }
        );
        const lastAccessIST = new Date(user.lastaccess * 1000).toLocaleString(
          "en-IN",
          { timeZone: "Asia/Kolkata" }
        );

        const lastCourseAccessIST = new Date(
          user.lastcourseaccess * 1000
        ).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });

        const firstDate = new Date(Date.parse(lastCourseAccessIST));
        const lastDateToAccess = new Date(Date.parse(startTimeIST));
        const totalDays =
          (lastDateToAccess - firstDate) / (1000 * 60 * 60 * 24);

        const firstAccess = new Date(firstDate);
        console.log(firstAccess, "firstAccess");
        const lastAccess = new Date(lastCourseAccessIST);
        const differenceInMs = lastAccess - firstAccess;

        // Convert milliseconds to days
        const millisecondsInDay = 1000 * 60 * 60 * 24;
        const differenceInDays = Math.floor(differenceInMs / millisecondsInDay);
        console.log(differenceInDays, "differenceInDays");
        // Update user object with converted timestamps
        user.firstaccess = startTimeIST;
        user.lastcourseaccess = lastCourseAccessIST;
        user.lastaccess = lastAccessIST;
        user.totalDays = differenceInDays;
