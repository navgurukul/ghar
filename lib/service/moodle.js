...
// ...............userEmail make as a key..............................
  ...
  async  topicAndActivitiesTest(emails, courseId, checkProdOrDev) {
    try {
      ...
      let totalCompletedActivityOfTheModule = 0;
      const userData = {};
      const userResults = await Promise.all(emails.map(async (email) => {
        ...
        for(let topic of response){
          ...
          for(let activity of topic.modules){
            ...
            if(activity.grade.status=="true"){
              completedActivity++;
              totalCompletedActivityOfTheModule++;
              if(new Date(activity.grade.timecompleted)>(new Date(latestTime))){
                latestTime = activity.grade.timecompleted
              }
            }
        ...
        console.log(totalActivityOfThisModule,'totalActivityOfThisModule')
      ...
      for (const email in userData) {
        if (userData.hasOwnProperty(email)) {
          if (userData[email].finalResponse) {
            ...
            result[email].totalTopicsCount = completed.split("||").length + notCompletedTopic.split("||").length;
            result[email].completedTopicCount = completed.split("||").length;
            result[email].totalActivityOfThisModule = totalActivityOfThisModule;
            result[email].totalCompletedActivityOfThisModule = totalCompletedActivityOfTheModule;
          } else {
            result[email] = {
