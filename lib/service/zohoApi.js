...
class ZohoCalculation {
  ...
  async getStudentDetails(min_value, max_value) {
    try {
      const params = {
        criteria:
          "Student_ID1 >=" + min_value + "&& Student_ID1 <=" + max_value,
      ...
      const getStudents = "Vijay_test_report";
      const studentsData = await zohoDataForCopyOf(getStudents, params);
      for (let i in studentsData.data) {
        if (studentsData.data[i].Select_School1 === "School of Programming") {
            studentsData.data[i].Select_School1 = "School of Management";
        } else if (studentsData.data[i].Select_School1 === "School of Management") {
            studentsData.data[i].Select_School1 = "School of Programming";
        }
    }    
      return studentsData;
    } catch (error) {
